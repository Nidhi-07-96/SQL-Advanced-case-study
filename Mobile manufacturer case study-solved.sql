...
WHERE YEAR(FT.Date) BETWEEN 2005 AND YEAR(GETDATE());
...
SELECT TOP 1 LOC.Country, LOC.State, COUNT(*) AS SamsungSales
FROM FACT_TRANSACTIONS FT
INNER JOIN DIM_LOCATION LOC ON FT.IDLocation = LOC.IDLocation
INNER JOIN DIM_MODEL M1 ON FT.IDModel = M1.IDModel
INNER JOIN DIM_MANUFACTURER MF1 ON M1.IDManufacturer = MF1.IDManufacturer
WHERE 
MF1.Manufacturer_Name = 'Samsung' AND LOC.Country = 'US'
...
FROM FACT_TRANSACTIONS FT
INNER JOIN 
DIM_MODEL M1 ON FT.IDModel = M1.IDModel
INNER JOIN 
DIM_LOCATION LOC ON FT.IDLocation = LOC.IDLocation
GROUP BY LOC.State, LOC.ZipCode, M1.Model_Name
...
SELECT 
    M1.Model_Name,
    MF1.Manufacturer_Name,
    AVG(FT.TotalPrice) AS AveragePrice
FROM 
    FACT_TRANSACTIONS FT
INNER JOIN 
    DIM_MODEL M1 ON FT.IDModel = M1.IDModel
INNER JOIN 
    DIM_MANUFACTURER MF1 ON M1.IDManufacturer = MF1.IDManufacturer
WHERE 
    MF1.IDManufacturer IN (
        SELECT TOP 5 
            MF1.IDManufacturer
        FROM 
            FACT_TRANSACTIONS FT
        ...
        INNER JOIN 
            DIM_MANUFACTURER MF1 ON M1.IDManufacturer = MF1.IDManufacturer
        GROUP BY 
            MF1.IDManufacturer, MF1.Manufacturer_Name
        ORDER BY 
            SUM(FT.Quantity) DESC
    )
GROUP BY 
    M1.Model_Name, MF1.Manufacturer_Name
ORDER BY 
    AveragePrice DESC;
...
SELECT 
dc.Customer_Name, 
AVG(FT.TotalPrice) AS avg_amt_spent
FROM DIM_CUSTOMER dc
JOIN 
Fact_Transactions FT ON dc.IDCustomer = FT.IDCustomer
...
SELECT * 
FROM (SELECT TOP 5 M1.Model_Name 
FROM FACT_TRANSACTIONS  FT 
INNER JOIN
DIM_MODEL M1 ON FT.IDModel=M1.IDModel
...
GROUP BY M1.Model_Name
ORDER BY SUM(FT.Quantity) DESC) AS TBL 
...
SELECT Manufacturer_Name, Year_wise_sales
FROM (
SELECT 
MF1.Manufacturer_Name,
YEAR(FT.Date) Year_wise_sales,
DENSE_RANK() OVER (ORDER BY SUM(FT.TotalPrice) DESC) AS Rank_1
FROM 
FACT_TRANSACTIONS FT
INNER JOIN 
DIM_MODEL M1 ON FT.IDModel = M1.IDModel
...
YEAR(FT.Date) = 2009
GROUP BY 
MF1.Manufacturer_Name, YEAR(FT.Date)
) AS Sales_2009
WHERE 
Rank_1 = 2
...
SELECT 
MF1.Manufacturer_Name,
YEAR(FT.Date) AS Year_wise_sales,
DENSE_RANK() OVER (ORDER BY SUM(FT.TotalPrice) DESC) AS Rank_1
FROM 
FACT_TRANSACTIONS FT
INNER JOIN 
...
WHERE YEAR(FT.Date) = 2010
EXCEPT
SELECT DISTINCT MF1.Manufacturer_Name
FROM FACT_TRANSACTIONS FT
INNER JOIN DIM_MODEL M1 ON FT.IDModel = M1.IDModel
INNER JOIN DIM_MANUFACTURER MF1 ON M1.IDManufacturer = MF1.IDManufacturer
WHERE YEAR(FT.Date) = 2009;
...
GROUP BY dc.Customer_Name, FT.IDCustomer
...
TC.Customer_Name, 
FT1.IDCustomer, 
YEAR(FT1.DATE) YEAR_1,
AVG(FT1.Quantity) Avg_Qty, 
AVG(FT1.TotalPrice) Avg_spend
FROM FACT_TRANSACTIONS FT1  
INNER JOIN TOP_CUST AS TC ON TC.IDCustomer = FT1.IDCustomer
GROUP BY TC.Customer_Name, FT1.IDCustomer, YEAR(FT1.DATE)
)
SELECT 
Customer_Name, 
YEAR_1, 
Avg_Qty, 
Avg_spend,
((Avg_spend - LAG(Avg_spend, 1) OVER (PARTITION BY IDCustomer ORDER BY YEAR_1)) / Avg_spend) * 100 AS Percent_Change 
FROM AVERAGE
ORDER BY IDCustomer, YEAR_1;
